...
void CountLrL1Server::updateWeight(const MessagePtr& msg) {
...
void CountLrL1Server::updateWeight(
    int grp, SizeR range, SArray<double> G) {
  ...
  double delta_max = conf_.countlrl1().delta_max_value();
  ...
  for (size_t i = 0; i < range.size(); ++i) {
    ...
    if (fabs(g)>1e-10) count_[grp][k] += 1;
    double learn_rate = eta/sqrt(count_[grp][k]+1);
    ...
    if (g_pos * learn_rate <= w) {
      //d = - g_pos / u;
      d = - g_pos * learn_rate;
    } else if (g_neg * learn_rate >= w) {
      //d = - g_neg / u;
      d = - g_neg * learn_rate;
    }
    ...
    //double lr = fabs(1.00/u);
    max_lr_ = std::max(max_lr_, learn_rate);
    min_lr_ = std::min(min_lr_, learn_rate);
...
void CountLrL1Server::evaluateProgress(Progress* prog) {
  size_t nnz_w = 0;
  size_t nnz_as = 0;
  double objv = 0;
  for (int grp : fea_grp_) {
