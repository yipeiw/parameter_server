...
void LrL2Worker::preprocessData(const MessagePtr& msg) {
  ...
  if (conf_.lrl2().has_sample_filter()) {
    ...
    //sample_filter_.init(conf_.lrl2().sample_filter());
  }
...
void LrL2Worker::pullAndUpdateDual(
...
void LrL2Worker::computeGradient(
    int grp, SizeR col_range, SArray<double> D) {
  CHECK_EQ(D.size(), col_range.size());
...
void LrL2Worker::updateDual(int grp, SizeR col_range, SArray<double> new_w) {
...
void LrL2Worker::evaluateProgress(Progress* prog) {
  busy_timer_.start();
  mu_.lock();  // lock the dual_
