...
void CountLrL1Worker::preprocessData(const MessagePtr& msg) {
  ...
  for (int grp : fea_grp_) {
    ...
    delta_[grp].resize(n, conf_.countlrl1().delta_init_value());
  }
  if (conf_.countlrl1().has_kkt_filter()) {
    using_kkt_filter_ = true;
    kkt_filter_.init(model_, fea_grp_);
  }
  
  if (conf_.countlrl1().has_round_filter() ) {
    using_round_filter_ = true;
    auto round_conf = conf_.countlrl1().round_filter();
...
void CountLrL1Worker::computeGradient(const MessagePtr& msg) {
...
void CountLrL1Worker::pullAndUpdateDual(
...
void CountLrL1Worker::computeAndPushGradient(
    int time, Range<Key> g_key_range, int grp, SizeR col_range) {
  ...
  if (!col_range.empty()) {
    ...
    for (int i = 0; i < npart; ++i) {
      ...
      pool.add([this, grp, thr_range, gr, &G]() {
          computeGradient(grp, thr_range, G.segment(gr));
        });
    }
    pool.startWorkers();
  }
...
void CountLrL1Worker::computeGradient(
    int grp, SizeR col_range, SArray<double> G) {
...
void CountLrL1Worker::updateDual(int grp, SizeR col_range, SArray<double> new_w) {
  ...
  for (size_t i = 0; i < new_w.size(); ++i) {
    ...
    double& cw = cur_w[j];
    double& nw = new_w[i];
    if (using_kkt_filter_ && kkt_filter_.marked(nw)) {
      kkt_filter_.clear(grp, j);
      cw = 0;
      delta_w[i] = 0;
      continue;
    }
...
void CountLrL1Worker::updateDual(
  ...
  for (size_t j = 0; j < X->cols(); ++j) {
    ...
    size_t n = offset[j+1] - offset[j];
    double wd = w_delta[j];
    if (using_kkt_filter_ && (wd == 0 || !kkt_filter_.test(grp, k)) ) {
      index += n;
      continue;
    }
...
void CountLrL1Worker::evaluateProgress(Progress* prog) {
  busy_timer_.start();
  mu_.lock();  // lock the dual_
