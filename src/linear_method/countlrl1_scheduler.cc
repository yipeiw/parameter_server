...
void CountLrL1Scheduler::init() {
  ...
  if (conf_.countlrl1().has_kkt_filter()) {
    ...
    kkt_filter_.set_ratio(conf_.countlrl1().kkt_filter().threshold_ratio());
  }
...
void CountLrL1Scheduler::runIteration() {
  CHECK(conf_.has_countlrl1());
...
void CountLrL1Scheduler::showKKTFilter(int iter) {
  if (iter == -3) {
...
void CountLrL1Scheduler::showLR(int iter) {
  if (iter == -3) {
...
void CountLrL1Scheduler::showRoundFilter(int iter) {
  if (iter == -3) {
...
void CountLrL1Scheduler::showProgress(int iter) {
  int s = iter == 0 ? -3 : iter;
